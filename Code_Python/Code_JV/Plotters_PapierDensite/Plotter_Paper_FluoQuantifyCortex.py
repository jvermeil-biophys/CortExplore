...
srcDir = f'C:/Users/josep/Documents/MagneticPincherData/Raw/{date2}_Chameleon/{specifFolder}Clean_Fluo_BF/'#'M1/'
...
cell = 'C11'#'C2'
cellId = f'{date}_{cell}'
specif = '_off4um' # '_off4um' #'_off4um' #'_off4um'
specifFolder = '' # 'M2-20um/B5mT/' # 'M2-20um/B5mT/' # 'M2-20um/Bstairs/' # 'M2-20um/B5mT/'
...
figSavePath = "C:/Users/josep/Desktop/Seafile/PapierDensité/DraftsFigs/F3/ExampleAnalysis"
...
def plotActinQuantity(cellId, t, profileMatrix, dfContact,
    ...
    ufun.archiveFig(figM, name = f'ProfileMetrics_t{t+1:.0f}_V2', ext = '.pdf', dpi = 100,
                    figDir = figSavePath, figSubDir = 'E-h-Fluo', cloudSave = 'flexible')
...
def actinQuantity(cellId, t, profileMatrix, dfContact,
    ...
    if PLOT_M:
        ...
        figM2, axM2 = plt.subplots(1, 1, figsize=(3.5/cm_in, 4.5/cm_in), layout='compressed')
        ...
        for ax in [axM[2], axM2]:
                    
            ax.plot(np.arange(Np)/7.4588, profile_n)
            ax.axvline(eR/7.4588, c='r', ls='--', lw=0.75, label='Cortex pos')
            # 1
            # ax.axvline(eR-n_in/7.4588, c='cyan', ls='--', lw=1)
            # ax.axvline((eR+n_out+1)/7.4588, c='cyan', ls='--', lw=1, label=f'Fixed boundaries: Q={Q_fbL:.1f}')
            # 2
            ...
            # ax.set_title('Actin quantification')
            ax.set_ylabel(f'Normalized intensity', labelpad=0.2, fontsize=7)
            ax.set_xlabel(f'r (µm)', labelpad=0.5, fontsize=7)
            ax.tick_params(axis='both', which='major')
            ax.xaxis.set_tick_params(pad=0.5, length=2.5)
            ax.yaxis.set_tick_params(pad=0.5, length=2.5)
            ax.grid(axis = 'y')
            ax.set_xlim([0, 12.5])
            ax.set_ylim([0, 2.25])
            # if z==nZ_scan-1:
        ...
        ufun.archiveFig(figM, name = f'ProfileMetrics_t{t+1:.0f}', ext = '.pdf', dpi = 300,
                        figDir = figSavePath, figSubDir = 'S3', cloudSave = 'flexible')
        ufun.archiveFig(figM2, name = f'ProfileMetrics_t{t+1:.0f}', ext = '.png', dpi = 500,
                        figDir = figSavePath, figSubDir = 'S3', cloudSave = 'flexible')
        ufun.archiveFig(figM2, name = f'ProfileMetrics_t{t+1:.0f}', ext = '.pdf', dpi = 300,
                        figDir = figSavePath, figSubDir = 'S3', cloudSave = 'flexible')
...
dfContact = actinQuantity(cellId, 1, profileMatrix, dfContact,
                          fluoCell, fluoCyto, fluoBack, fluoBeadIn,
...
fig, ax = plt.subplots(1, 1, figsize = (3.0/cm_in, 4.5/cm_in), layout='compressed')#, layout="constrained")
...
#### Plot 1
fig, ax = plt.subplots(1, 1, figsize=(3/cm_in, 4.5/cm_in), 
                       layout='compressed')#, layout='compressed')
...
sns.swarmplot(data = df_res, ax = ax, x='manipID', y='expo',
              size = 4.0, color = apm.cL_Set2[0], hue = 'cellID', palette=Palette, 
...
ax.errorbar([0, 1, 2, 3], df_res_g['expo_mean'].values, 
            ls='', marker='_', markerfacecolor=(1, 1, 1, 0.0),
            mec = ebarcolor, mew = 1.5*a, ms = 12*a,
            xerr=None, yerr=df_res_g['expo_ste'].values,
...
# %%% F3E_V3 - Plot exponents for all cells LIN/LOG
...
ax.axhline(np.median(df_res['expo'].values), color = 'darkred', lw=1.5, 
           label=f"Median = {np.median(df_res['expo'].values):.2f}", alpha = 0.8)
sns.swarmplot(data = df_res, ax = ax, x='manipID', y='expo',
              size = 4.0, color = apm.cL_Set2[0], hue = 'cellID', palette=Palette, 
              # color = apm.cL_Set2[0], hue = 'cellID', palette=Palette, 
...
rp = cm_in
fig, axes = plt.subplots(1, 2, figsize = (17.0/rp, 8/rp), layout="compressed")
...
ax.plot(Xplot, Yplot, ls = '-', c = 'dimgray', lw = 1.5, zorder=8)
...
ax.set_prop_cycle(color=cL_f3)
ax.scatter(X, Y, c=C, marker='o', s=30, zorder=3, cmap = cM_f3) # , style='cellNum'
...
for k, data in enumerate([X, Y]): # +'_wAvg'
    ...
    ax.plot([], [], label=f'N: {shap_pval:.2e}', ls='', marker='o', 
