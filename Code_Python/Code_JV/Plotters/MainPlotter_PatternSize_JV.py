...
invDictSubstrates = {}
for i in range(5,105,5):
    invDictSubstrates[str(i) + 'um fibronectin discs'] = i
...
GlobalTable_meca_DB = taka.getMergedTable('Global_MecaData_DB', mergeUMS = True)

adh_diam = GlobalTable_meca_DB['substrate'].apply(lambda x : invDictSubstrates[x])
GlobalTable_meca_DB.insert(6, 'adhesion diameter', adh_diam)

cols = GlobalTable_meca_DB.columns
...
# %%% Pattern size effect [DUYA'S INTERNSHIP]


# %%%% Only A11 only H0

data = GlobalTable_meca_DB
# dates = ['21-01-18', '21-01-21']
# dates = ['22-07-22', '22-07-29', '22-08-24']
dates = ['22-07-22', '22-07-29', '22-08-24']
...
fig, ax, dfcount, dfcountcells = D1Plot(data, CondCols=['adhesion diameter'], Parameters=[thicknessType],Filters=Filters,
                 AvgPerCell=False, cellID='cellID', co_order=co_order, stats=False, statMethod='Mann-Whitney', 
                 box_pairs=box_pairs, figSizeFactor = 1.5, markersizeFactor = 0.8, orientation = 'v', stressBoxPlot=2,
                 returnCount = 2)

renameAxes(ax,renameDict1)
ax[0].set_xlabel('Disc diameter (µm)')
...
fig, ax, dfcount, dfcountcells = D1Plot(data, CondCols=['adhesion diameter'], Parameters=[thicknessType],Filters=Filters,
                 ...
                 box_pairs=box_pairs, figSizeFactor = 1.5, markersizeFactor = 1.35, orientation = 'v', stressBoxPlot=1,
                 returnCount = 2)

renameAxes(ax,renameDict1)
ax[0].set_xlabel('Disc diameter (µm)')
fig.suptitle('Cortical thickness & Adhesion state')
