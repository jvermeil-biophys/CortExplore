...
L1_full = 127000; % L stands for length
M1 = 5000; % M stands for margin
L1 = L1_full - 2*M1;
O1_full = - L1_full/2; % O stand for origin
O1 = O1_full + M1;
R1 = 4; % C stands for rows
C1 = 4; % C stands for columns
...
circle_radii = [10, 15, 20, 25];
circle_spacing = [100, 120, 140, 160];
...
for x1=0:C1-1
    for y1=0:R1-1
        ...
        O2y = O1 + y1*L2_full + M2;
        if y1 == 0
            circle_r = circle_radii(x1+1);
            circle_s = circle_spacing(x1+1);
            N = floor(L2/circle_s);
            for x2=0:N-1
                for y2=1:N-1
                    circle(O2x + x2*circle_s, O2y + y2*circle_s, circle_r);
                end
            end
        end
    end
